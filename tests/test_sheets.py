...
def test_a_fresh_sheet_has_empty_field_values() -> None:
    assert SheetSet().at(0).field_values == {}


def test_set_field_values_replaces_the_sheet_in_place() -> None:
    sheets = SheetSet()
    values = {"powiat": "wrocławski", "gmina": "Kobierzyce", "dz_nr": "394"}
    sheets.set_field_values(0, values)
    assert sheets.at(0).field_values == values


def test_field_values_survive_a_state_round_trip() -> None:
    values = {"powiat": "trzebnicki", "gmina": "Trzebnica", "obreb": "KOMOROWO", "dz_nr": "34/1", "sketch_number": "2"}
    sheet = Sheet(name="Sheet 1", field_values=values)
    assert sheet_from_state(state_from_sheet(sheet)) == sheet


def test_the_whole_set_with_field_values_survives_a_state_round_trip() -> None:
    sheets = SheetSet()
